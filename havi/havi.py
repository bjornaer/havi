...
from havi.physics.model import EulerAngles, Unit, Wave, ProbabilisticVariable
from havi.bayes.inference import get_model, mcmc

Tensor = torch.Tensor

class Havi:
    def __init__(self, incident_angles: List[float], scatered_angles: List[float], wavelength: float) -> None:
    ...
    def set_boundaries(self, rms_height: Tuple[float], correlation_longitude: Tuple[float], dielectric_constant: Tuple[float]) -> None:
        """
        ...
        self.rms_height = ProbabilisticVariable("rms height", Unit.METER, rms_height[0], rms_height[1])
        self.correlation_longitude = ProbabilisticVariable("correlation longitude", Unit.METER, correlation_longitude[0], correlation_longitude[1])
        self.dielectric_constant = ProbabilisticVariable("epsilon", Unit.UNITLESS, dielectric_constant[0], dielectric_constant[1])
        self._boudaries_set = True

    def inference(self, sigma, observed) -> Union[dict, Any, None]:
        if not self._boudaries_set:
            raise Exception("boundaries for probabilistic variables not set! please call set_boundaries method")
        model = get_model(self.dielectric_constant, self.rms_height, self.correlation_longitude, self.incident_wave, self.scatered_wave)
